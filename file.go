...
func decryptFile(passphrase []byte, input *os.File, finalOutput string) (err error) {
	defer func() {
		if r := recover(); r != nil {
			err = fmt.Errorf("decryptFile encountered an error: %v", r)
		}
	}()
...
func encryptFile(passphrase []byte, input *os.File, finalOutput string) (err error) {
	defer func() {
		if r := recover(); r != nil {
			err = fmt.Errorf("encryptFile encountered an error: %v", r)
		}
	}()
